		...
		if (showMarketplace) {
			McpServiceClient.refreshMcpMarketplace(EmptyRequest.create({}))
				...
				.catch((error) => {
					...
					setError(t("mcp.marketplace.loadError"))
					setIsLoading(false)
					setIsRefreshing(false)
				})
		}
			...
			<div style={{ padding: "20px 20px 5px", display: "flex", flexDirection: "column", gap: "16px" }}>
				...
				<div
					...
					<VSCodeRadioGroup
						...
						value={sortBy}>
						<VSCodeRadio value="downloadCount">{t("mcp.marketplace.sortMostInstalls")}</VSCodeRadio>
						<VSCodeRadio value="newest">{t("mcp.marketplace.sortNewest")}</VSCodeRadio>
						<VSCodeRadio value="stars">{t("mcp.marketplace.sortStars")}</VSCodeRadio>
						<VSCodeRadio value="name">{t("mcp.marketplace.sortName")}</VSCodeRadio>
					</VSCodeRadioGroup>
				</div>
			</div>

	...
	return (
		<div className="flex flex-col gap-2">
			...
			<OllamaModelPicker
				...
				placeholder={ollamaModels.length > 0 ? t("api.model.searchPlaceholder") : "e.g. llama3.1"}
				selectedModelId={ollamaModelId || ""}
			/>

			{/* Show status message based on model availability */}
			{ollamaModels.length === 0 && <p className="text-sm mt-1 text-description italic">{t("api.ollama.fetchError")}</p>}
			...
			<DebouncedTextField
				...
				style={{ width: "100%" }}>
				<span className="font-semibold">{t("api.contextWindow")}</span>
			</DebouncedTextField>
			...
			{showModelOptions && (
				<>
					<DebouncedTextField
						...
						style={{ width: "100%" }}>
						<span className="font-semibold">{t("api.requestTimeout")}</span>
					</DebouncedTextField>
					<p className="text-xs mt-0 text-description">{t("api.requestTimeout.description")}</p>
				</>
			)}
			...
			<p
				...
				{t("api.ollama.description")}{" "}
				<VSCodeLink
					href="https://github.com/ollama/ollama/blob/main/README.md"
					style={{ display: "inline", fontSize: "inherit" }}>
					{t("api.ollama.quickstartGuide")}
				</VSCodeLink>{" "}
				<span style={{ color: "var(--vscode-errorForeground)" }}>
					(<span style={{ fontWeight: 500 }}>{t("api.ollama.note")}</span> {t("api.ollama.noteText")})
				</span>
			</p>
		</div>
	)

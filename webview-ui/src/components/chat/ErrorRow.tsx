		...
		switch (errorType) {
				...
				if (rawApiError) {
					...
					if (clineError?.isErrorType(ClineErrorType.Auth) && isClineProvider) {
						return !clineUser ? (
							...
							<div className="mt-4">
								<span className="text-description">(아래의 "재시도"를 클릭하세요)</span>
							</div>
						)
					}
					...
					return (
						<p className="m-0 whitespace-pre-wrap text-error wrap-anywhere flex flex-col gap-3">
							...
							{errorMessage?.toLowerCase()?.includes("powershell") && (
								<div>
									Windows PowerShell 문제가 발생한 것 같습니다.{" "}
									<a
			...
			case "diff_error":
				return (
					<div className="flex flex-col p-2 rounded text-xs opacity-80 bg-quote text-foreground">
						<div>모델이 파일에서 일치하는 검색 패턴을 찾지 못했습니다. 재시도 중...</div>
					</div>
				)
			...
			case "clineignore_error":
				return (
					<div className="flex flex-col p-2 rounded text-xs opacity-80 bg-quote text-foreground">
						<div>
							Cline이 <code>{message.text}</code>에 접근하려 했지만 <code>.clineignore</code> 파일에 의해
							차단되었습니다.
						</div>
					</div>
				)
